...
from diffusers import AutoencoderKL, StableDiffusionPipeline, UNet2DConditionModel, ControlNetModel
...
class IDAvatarCLIPImageEncoder(nn.Module):
    ...
    def from_pretrained(
        ...
        model,_,_ = open_clip.create_model_and_transforms('ViT-L-14', pretrained=global_model_name_or_path)
    ...
    def forward(self, person_pixel_values):
        ...
        person_embeds, patch_features = self.vision_model(person_pixel_values)# b, 768; b, 256, 1024
        person_embeds = person_embeds.view(b, 1, -1) # b, 1, 768
        return person_embeds, patch_features


def fuse_object_embeddings(
    inputs_embeds, # b, 77, 768
    image_token_mask, # b, 77
    object_embeds, # b, 1, 768
    alpha=1,
):
    ...
    inputs_embeds = inputs_embeds.view(-1, inputs_embeds.shape[-1]) # b*77, 768
    image_token_mask = image_token_mask.view(-1) # b*77
    valid_object_embeds = object_embeds.view(-1, object_embeds.shape[-1]) # b*num_objs, 768

    # slice out the image token embeddings
    # image_token_embeds = inputs_embeds[image_token_mask] # b*77, 768 chose the only one image_token_embeds - img
    ...
    outputs_embeds = outputs_embeds.view(batch_size, seq_length, -1) # bsz, 77, 768
...
class IDAvatarModel(nn.Module):
    def __init__(self, text_encoder, image_encoder, vae, unet, controlnet, args):
        ...
        self.patch_mlp = PatchMLP(1024, embed_dim)
    ...
    def from_pretrained(args):
        ...
        unet = UNetModel(fuser_scale=args.fuse_scale)
    ...
    def forward(self, batch, noise_scheduler):
        ...
        # (bsz, num_image_tokens, dim) (bsz, 1, 768) and (bsz, 256, 1024)
