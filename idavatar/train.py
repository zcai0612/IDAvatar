...
from idavatar.data import IDAvatarDataset, get_data_loader

from idavatar.transforms import (
...
def train():
    ...
    for name, p in model.unet.named_parameters():
        if ("transformer_blocks" in name) and ("fuser" in name):
            ...
            p.to(torch.float32)
            unet_trainable_params.append(p)
    
    model.postfuse_module.requires_grad_(True)
    model.postfuse_module.to(torch.float32)
    postfuse_params = [p for _, p in model.postfuse_module.named_parameters() if p.requires_grad]

    parameters = unet_trainable_params + postfuse_params
    ...
    optimizer = optimizer_cls(
        [
            {'params': unet_trainable_params, 'lr': args.learning_rate * 0.01},
            {'params': postfuse_params, 'lr': args.learning_rate},
        ],
    ...
    train_dataset = IDAvatarDataset(
        images_dir=args.images_dir,
        masks_dir=args.masks_dir,
        prompts_path=args.prompts_path,
        prompts_aug_path=args.prompts_aug_path,
        tokenizer=tokenizer,
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("idavatar", config=vars(args))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model), torch.backends.cuda.sdp_kernel(
                ...
                accelerator.backward(denoise_loss)
